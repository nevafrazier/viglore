...
ACHIEVEMENT_SIGNALS = [
    r'best.?sell', r'highest.?gross', r'guinness world record',
    r'[\d,.]+\s*(?:million|billion)\s*copies',
    r'[\d,.]+\s*(?:million|billion)\s*(?:monthly active\s*)?(?:players|users|registered)',
    r'number.?one\b', r'\d+\s*(?:platinum|gold)\s*(?:certif|record)',
    r'inducted into the',
...
def extract_achievements(full_text: str) -> list[str]:
    ...
    for raw in sentences:
        ...
        if len(s) < 25 or len(s) > 300:
            continue
        lower = s.lower()
        for sig in ACHIEVEMENT_SIGNALS:
...
async def fetch_topic(q: str, title: str = None) -> dict:
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        try:
            summary_resp, full_resp = await _fetch_wikipedia_data(client, search_term)
            ...
            logger.warning("Wikipedia fetch failed for '%s': %s", search_term, e)
...
async def fetch_suggestions(q: str) -> list[dict]:
    headers = {"User-Agent": "Viglore/1.0 (https://github.com/nevafrazier/viglore) httpx/0.27"}
    async with httpx.AsyncClient(headers=headers) as client:
        try:
            ...
            if resp.status_code == 200:
                ...
                return [{"title": t, "description": d} for t, d in zip(titles, descs) if t]
        except Exception as e:
            logger.warning("Wikipedia suggestions failed for '%s': %s", q, e)
    return []


async def _fetch_wikipedia_data(client: httpx.AsyncClient, search_term: str):
